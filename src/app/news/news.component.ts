...
export class NewsComponent implements OnInit, OnChanges {
  ...
  ngOnInit() {
    this.subscribers.filtersUpdated$ = this.appService.isFilterUpdated$.subscribe(
      value => {
  ...
  getAllHeadlines(filters) {
    const params = filters.length
      ? filters.map(v => v.param).reduce((a, b) => a + b)
      : '';
    this.articles = [];
    this.subscribers.getAllHeadlines = this.newsService
      .getTopHeadings(params)
      .subscribe(
        (value: any) => {
          if (
            value.status === 'error' ||
            (value.articles && !value.articles.length)
          ) {
  ...
  getAllSources(filters) {
    const params = filters
      .filter(item => item.key !== 'q')
      .map(v => v.param)
      .reduce((a, b) => a + b);
    this.sources = [];
    this.subscribers.getAllSources$ = this.newsService
      .getSources(params)
      .subscribe(
        (value: any) => {
          if (
            value.status === 'error' ||
            (value.sources && !value.sources.length)
          ) {
